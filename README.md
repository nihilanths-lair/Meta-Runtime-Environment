...
* **Разработка расширений:** Сторонние разработчики имеют право создавать и распространять исключительно **зависимые языковые пакеты (модули расширения мета-синтаксиса)**. Любой созданный на платформе DSL/GPL-язык не может функционировать как независимая отдельная программа (stand-alone компилятор) и должен распространяться только в виде бинарного пакета расширения, исполняемого строго внутри Modifiable Modular Infrastructure Providing Development Tools (MMIPDT).